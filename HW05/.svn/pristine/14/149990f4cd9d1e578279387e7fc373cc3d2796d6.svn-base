...
/**
 * An observer for IDispatcher.
 *
 * When registered with a dispatcher, an observer will receive the TDispMsg type
 * message that the dispatcher sends to its registered observers.
 *
 * @param <TDispMsg> The type of message that this observer can process
 */
public interface IObserver<TDispMsg> {
  /**
   * The method that the dispatcher will call to process the supplied message.
   *
