...
/**
 * Following defines the view of the RMI server system.
 * 
 * @author Xioajun Wu 
 * @author Haoshan Zhou
 */
public class RMIServerFrame extends JFrame {
	...
	/**
	 * The control panel that holds quit button and sent message text area.
	 */
	private final JPanel panelCtrl = new JPanel();

	/**
	 * The button that is used to quit the server.
	 */
	private final JButton btnQuit = new JButton("Quit\n");

	/**
	 * The scroll panel that holds sent message text area.
	 */
	private final JScrollPane scrollPaneOfSendText = new JScrollPane();

	/**
	 * The text area that user can type in and then press enter key to sent typed message to client.
	 */
	private final JTextArea textAreaOfSendedMsg = new JTextArea(1, 25);

	/**
	 * The label of sent message text area.
	 */
	private final JLabel lblSenfMsg = new JLabel("Send msg to remote client's view");

	/**
	 * The scroll panel that holds the RMI server console text area.
	 */
	private final JScrollPane scrollPaneOfRMIServerConsole = new JScrollPane();

	/**
	 * The text area that represents the RMI server console.
	 */
	private final JTextArea textAreaRMIServerConsole = new JTextArea();

	/**
	 * The adapter that this view used to communicate with the model.
	 */
	 ...
	 * 
	 * @param view2ModelAdapter The view to model adapter that the view used to communicate with model.
	 */
	...
	public void init() {
		...
		panelCtrl.add(scrollPaneOfSendText);
		...
		//display pannel setting:
		contentPane.add(scrollPaneOfRMIServerConsole, BorderLayout.CENTER);
		textAreaRMIServerConsole.setWrapStyleWord(true);
		textAreaRMIServerConsole.setLineWrap(true);

		scrollPaneOfRMIServerConsole.setViewportView(textAreaRMIServerConsole);
	}
	...
	public void addText(String newText) {
		...
		textAreaRMIServerConsole.setCaretPosition(textAreaRMIServerConsole.getDocument().getLength());
	}
}
